...
export default (str: any, config: any = {}) => {
  ...
  if (toHTML) {
    ...
    each(scripts, (node: any) => body.appendChild(node));
    // Move inside body all head children
    const hEls: any[] = [];
    each(head.children, (n: any) => hEls.push(n));
    each(hEls, (node: any, i: any) =>
      body.insertBefore(node, body.children[i])
    );
    res = body;
  } else {
    res = doc.firstChild;
