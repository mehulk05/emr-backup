...
@Component({
  selector: 'app-media-file-folder-list',
  templateUrl: './media-file-folder-list.component.html',
  styleUrls: ['./media-file-folder-list.component.css']
})
export class MediaFileFolderListComponent implements OnInit {
  globalFilterColumn = [
    'id',
    'filename',
    'name',
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe((data: any) => {
      ...
      if (data.subfolderId) {
        ...
        this.getFilesByFolderId(this.selectedTag);
      }
    });
  }
  ...
  getTags() {
    this.fileManagerService.socialMediaTagList().then(
      (response: any) => {
        this.tags = response;
      },
      (error) => {
  ...
  getFilesBySubFolderId() {
    this.fileManagerService
      .getFilesByFolderId(this.subfolderId, this.selectedTag)
      .then((data: any) => {
        this.folderList = data?.fileList.content;

        this.dataList = [...this.folderList];
      });
  }

  getFilesByFolderId(selectedTag: string) {
    this.fileManagerService
      .getFilesByFolderId(this.folderId, selectedTag)
      .then((data: any) => {
  ...
  openModalForTags(tagArray :any){
    this.tagsData = tagArray;
